...
def bounding_boxes_images(args, ROOT_DIR, DEFAULT_OID_DIR):
	...
	if args.command == 'all' or args.command == 'downloader':
		logo(args.command)
		...
		domain_dict = make_domain_list(name_file_path,domain_list) # create class list file for each domain in data/custom/domain_list directory
		mkdirs(dataset_dir, csv_dir, domain_dict)

		for domain_name, class_list  in domain_dict.items():
			print("domain_name:" + str(domain_name))
			print("Class_list:" +str(class_list[1:]))
			...
			for class_name in class_list[1:]:
					for i in range(2):
						...
						if not args.n_threads:
							download(args, data_type, df_val, folder[i], dataset_dir, class_name, class_dict[class_name], domain_name,domain_dict)
						else:

							download(args, data_type, df_val, folder[i], dataset_dir, class_name, class_dict[class_name], domain_name, domain_dict, args.n_threads)
